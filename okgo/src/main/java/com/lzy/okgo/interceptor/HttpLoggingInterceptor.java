...
public class HttpLoggingInterceptor implements Interceptor {
  ...
  public HttpLoggingInterceptor(String tag) { logger = Logger.getLogger(tag); }
  ...
  private void log(String message) { logger.log(colorLevel, message); }
    ...
    try {
      ...
      if (logHeaders) {
        if (hasRequestBody) {
          ...
          // interceptor. Force them to be included (when available) so there
          // values are known.
      ...
      if (logHeaders) {
        ...
        if (logBody && HttpHeaders.hasBody(clone)) {
          ...
          if (isPlaintext(responseBody.contentType())) {
            ...
            log("\tbody:" + body);
            responseBody =
                ResponseBody.create(responseBody.contentType(), bytes);
