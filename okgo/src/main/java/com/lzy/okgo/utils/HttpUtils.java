...
public class HttpUtils {
  ...
  public static Request.Builder appendHeaders(Request.Builder builder,
    ...
    try {
      for (Map.Entry<String, String> entry : headers.headersMap.entrySet()) {
        // 对头信息进行 utf-8
        // 编码,防止头信息传中文,这里暂时不编码,可能出现未知问题,如有需要自行编码
        //                String headerValue =
  ...
  public static RequestBody generateMultipartRequestBody(HttpParams params,
    ...
    if (params.fileParamsMap.isEmpty() && !isMultipart) {
      ...
      // 拼接文件
      for (Map.Entry<String, List<HttpParams.FileWrapper>> entry :
           params.fileParamsMap.entrySet()) {
